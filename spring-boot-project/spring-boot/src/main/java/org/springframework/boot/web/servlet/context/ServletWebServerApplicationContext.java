...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	protected void onRefresh() {
		super.onRefresh();
		try {
			//在此创建Tomcat，并且启动。到此还剩下一个问题DispatchServlet在哪里关联上的
			createWebServer();
		}
		catch (Throwable ex) {
	...
	private void createWebServer() {
		//这里开始创建WebServer，由于是初次创建，这里都是null
		WebServer webServer = this.webServer;
		ServletContext servletContext = getServletContext();
		if (webServer == null && servletContext == null) {
			//构造一个web工厂
			ServletWebServerFactory factory = getWebServerFactory();
			//进入这里，调用工厂方法，getWebServer()，最终是由TomcatServletWebServerFactory实现的具体方法
			// 所以进入TomcatServletWebServerFactory.getWebServer()
			this.webServer = factory.getWebServer(getSelfInitializer());
		}
		else if (servletContext != null) {
			try { //如果发现servletContext不是null，那么直接调用onStartup()方法
				// 这部分内容在《SpringBoot深度探究（一）模拟启动Tomcat》博客中有详细讲解
				// 使用了Service Provider Interface（简称SPI）技术
